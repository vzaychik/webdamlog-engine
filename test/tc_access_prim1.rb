...
fact local1@p1(3);
  ...
  def test_capc
    begin
      ...
      assert_equal [{:atom1=>"1", :priv=>"Read",:plist=>PList.new(["test_access","p1"].to_set)}, {:atom1=>"2",:priv=>"Read",:plist=>PList.new(["test_access","p1"].to_set)}],
        runner2.tables[:delegated1_i_ext_at_p1].map{|t| Hash[t.each_pair.to_a]}
      assert_equal [{:atom1=>"1", :atom2=>"3", :priv=>"Read", :plist=>PList.new(["p1"].to_set)},{:atom1=>"2",:atom2=>"3", :priv=>"Read", :plist=>PList.new(["p1"].to_set)}], runner2.tables[:delegated_join_i_ext_at_p1].map{ |t| Hash[t.each_pair.to_a] }
      ...
      assert_equal [{:atom1=>"1",:atom2=>"3", :priv=>"Read", :plist=>PList.new(["test_access","p1"].to_set)},{:atom1=>"2",:atom2=>"3", :priv=>"Read", :plist=>PList.new(["test_access","p1"].to_set)}], runner1.tables[:local3_i_ext_at_test_access].map{ |t| Hash[t.each_pair.to_a] }
      #incidentally, this also means delegated_join has an updated list
      assert_equal [{:atom1=>"1", :atom2=>"3", :priv=>"Read", :plist=>PList.new(["test_access","p1"].to_set)},{:atom1=>"2",:atom2=>"3", :priv=>"Read", :plist=>PList.new(["test_access","p1"].to_set)}], runner2.tables[:delegated_join_i_ext_at_p1].map{ |t| Hash[t.each_pair.to_a] }
